...
const $cube_commands = {
	...
	edit: {
		punish: (idx, type) => {
			if (isIdxValid(idx)) {
				return edit_punish(idx - 1, punish_type(type));
			} else {
				return [`Invalid index: ${idx}`, BAD];
			}
		},
	...
	remove: (idx) => {
		...
		const removeSingleIndex = (index) => {
			...
			logs();
			return [`Removed time on index ${index}`, GOOD];
		};
		...
		const removeNegativeIndex = (index) => {
			...
			if (isIdxValid(positiveIndex)) {
				...
				logs();
				return [`Removed time on index ${index} (${all_times.length + 2 + index})`, GOOD];
			} else {
				return [`Invalid index: ${idx}`, BAD];
			}
		};
		...
		// handle no arguments
		if (!idx) {
			return ["Remove what?", PROB];
		}

		// check if the input is to remove all elements
		if (idx.toLowerCase() === "all" || idx === "*") {
			const confirmation = confirm("Are you sure you want to remove all elements?");
			if (confirmation) {
				all_times = [];
				logs();
				return ["Removed all elements", GOOD];
			} else {
		...
		// check if the input is a negative index
		const index = parseInt(idx);
		if (!isNaN(index) && index < 0) {
			return removeNegativeIndex(index);
		}

		// check if the input is a single index
		if (!isNaN(index) && isIdxValid(index)) {
			return removeSingleIndex(index);
		} else {
			return [`Invalid index: ${idx}`, BAD];
		}
	},
	...
	session: {
		...
		add: (name = null) => {
			...
			createSelect(s_length, s_idxs, sessionSelect, (i, opt) => {
				return i == current_session;
			});
...
const $other_commands = {
	version: () => {
		return [`last update: ${last_update}`, GOOD];
	},
	refresh: () => {
		location.href = location.href;
	},
